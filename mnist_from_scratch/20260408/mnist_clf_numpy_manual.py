...
from common.functions import one_hot, sigmoid, sigmoid_grad, softmax, cross_entropy, accuracy
...
if __name__ == "__main__":
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for idx in range(0, len(x_train), BATCH_SIZE):
            ...
            # Backward propagation (multiclass: softmax + cross_entropy)
            # dout = cross_entory_grad(y_preds, y)
            # grad_z3 = softmax_grad(y_preds) * dout
            grad_z3 = (y_preds - y) / batch_size    
    ...
    for i in range(NUM_SAMPLES):
        print(f"Target: {y[i].argmax()} | Prediction: {y_preds[i].argmax()}")
